####Using Ames Skelton Code but revising to fit my analysis

library(ggplot2)

setwd("~/Google Drive/0. 18_SPRING_MSDS_410_RegressionAndMultivariateAnalysis")
...
# There are 5 observations that we may consider
# removing from the data set (a plot of SALE PRICE versus GR
#LIV AREA will indicate them quickly).
ggplot(mydata, aes(x=GrLivArea, y=SalePrice)) + 
    geom_point(color="blue", shape=1) +
    ggtitle("Scatter Plot of Ground Living Area vs Sale Price") +
    theme(plot.title=element_text(lineheight=0.8, face="bold", hjust=0.5))

ggplot(mydata, aes(x=SalePrice, y=GrLivArea)) + 
    geom_boxplot(fill="blue") +
    labs(title="Distribution of HouseAge") +
    theme(plot.title=element_text(lineheight=0.8, face="bold", hjust=0.5))

#Three of them are true
# outliers (Partial Sales that likely don't represent actual
# market values) and two of them are simply unusual sales 
# (very large houses priced relatively appropriately). 

# I would recommend removing any houses with more than 4000 
#square feet from the data set (which eliminates these 5
#unusual observations).  Will do this later on in the code.
...
ggplot(subdat, aes(x=SalePrice)) + 
  ...
  theme(plot.title=element_text(lineheight=0.8, face="bold", hjust=0.5))
#Findings: Heavy-tailed to the right
...
ggplot(subdat, aes(x=QualityIndex)) + 
  ...
  theme(plot.title=element_text(lineheight=0.8, face="bold", hjust=0.5))
#Findings: Looks to maybe be normally distributed
...
ggplot(subdat, aes(x=TotalFloorSF, y=QualityIndex)) + 
  ...
  theme(plot.title=element_text(lineheight=0.8, face="bold", hjust=0.5))
#Slight positive correlation but handful of outliers when 
#total floor sf > 4000 as discovered earlier.

ggplot(subdat, aes(x=TotalFloorSF, y=HouseAge)) + 
  geom_point(color="blue", shape=1) +
  ggtitle("Scatter Plot of Total Floor SF vs HouseAge") +
  theme(plot.title=element_text(lineheight=0.8, face="bold", hjust=0.5))
#Findings: No clear correlation visually

ggplot(subdat, aes(x=LotShape, y=HouseAge)) + 
  geom_boxplot(fill="blue") +
  labs(title="Distribution of HouseAge") +
  theme(plot.title=element_text(lineheight=0.8, face="bold", hjust=0.5))
#Findings: boxplots are useful for a mix of categorical and 
#quantatitative EDA.  Can quickly pinpoint outliers for IR2,
#and notice the difference in the mean and lower and upper
...
#Drawing the simple linear model here
...
#Based on outliers detected earlier, this may be one way of
#sampling the data
subdat2 <- subdat[which(subdat$TotalFloorSF < 4000),]

#Based on Drop Conditions identified in Assignment #1
subdat2 <- mydata[which(mydata$BldgType=="1Fam"  & 
                            (mydata$LotShape=="Reg" | 
                                 mydata$LotShape=="IR1") 
                        & (mydata$SaleCondition=="Normal")),]

#Checking if drop conditions were applied correctly
summary(subdat2$LotShape)
summary(subdat2$BldgType)
#using table here because summary doesn't display varchar 
#field well
table(subdat2$SaleCondition)

###################################################################
##################  Assignment 2  ################################
#################################################################

#Identify the columns that are continuous since I am going
#to be looking to create a simple linear regression model
#which requires a continuous predictor variable.
subdat2<-subdat2[,sapply(subdat2,is.numeric) | sapply(
    subdat2,is.integer)]
corr_table<-round(cor(subdat2),2)
SalePriceCorrCoef<-corr_table["SalePrice",]
sort(SalePriceCorrCoef,decreasing=TRUE)
...
ggplot(subdat2, aes(x=OverallQual, y=SalePrice)) + 
  geom_point(color="blue", shape=1) +
...
#Use these two predictor variables to fit two simple linear 
#regression models.  Produce the model equations and 
#interpret the coefficients. Produce the relevant diagnostic
#plots to assess the goodness-of-fit of each model.  We are
#looking for two particular residual plots. On what criteria
#are you assessing the model fit?  Include each model in 
#its own section of your report.  
...
SLRresult2 = lm(SalePrice ~ OverallQual, data=subdat2)
anova(SLRresult2)
summary(SLRresult2)
par(mfrow=c(2,2))  # visualize four graphs at once
plot(SLRresult2)
...
MLRresult = lm(SalePrice ~ TotalFloorSF+OverallQual, data=subdat2)
anova(MLRresult)
summary(MLRresult)
par(mfrow=c(2,2))  # visualize four graphs at once
plot(MLRresult)
...
##Using logSalePrice instead of SalePrice
SLRresult = lm(logSalePrice ~ TotalFloorSF, data=subdat2)
anova(SLRresult)
summary(SLRresult)
par(mfrow=c(2,2))  # visualize four graphs at once
plot(SLRresult)

SLRresult2 = lm(logSalePrice ~ OverallQual, data=subdat2)
anova(SLRresult2)
summary(SLRresult2)
par(mfrow=c(2,2))  # visualize four graphs at once
plot(SLRresult2)

MLRresult = lm(logSalePrice ~ TotalFloorSF+OverallQual, data=subdat2)
anova(MLRresult)
summary(MLRresult)
par(mfrow=c(2,2))  # visualize four graphs at once
plot(MLRresult)
